  ...
  const renderDay = (day: number) => {
    ...
    return (
      <button
        ...
        `}
      >
        <span className={`text-sm font-bold ${isSelected ? 'text-white dark:text-slate-900' : ''}`}>{day}</span>
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-7 gap-2 sm:gap-3 mb-6">
            {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => (
              <div key={i} className="text-center text-xs font-bold text-slate-400 dark:text-slate-500 py-2 uppercase tracking-wider">
                {d}
              </div>
            ))}
