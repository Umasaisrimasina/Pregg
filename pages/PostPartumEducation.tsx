  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
        {/* Hero */}
        <div className="xl:col-span-3 bg-white dark:bg-dm-card rounded-[2rem] p-8 lg:p-12 shadow-sm border border-slate-100 dark:border-dm-border overflow-hidden relative">
      ...
      {/* Recovery Topics */}
      <div>
        <h2 className="text-xl font-bold font-display text-slate-900 dark:text-dm-foreground mb-6">Recovery Topics</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            { icon: Heart, title: "Physical Recovery", desc: "Healing timelines and self-care", color: "text-purple-500", bg: "bg-purple-50" },
            { icon: Brain, title: "Mental Wellness", desc: "Understanding baby blues vs PPD", color: "text-secondary-500", bg: "bg-secondary-50" },
            { icon: Baby, title: "Newborn Bonding", desc: "Building attachment with your baby", color: "text-purple-600", bg: "bg-purple-50" },
            { icon: Shield, title: "Pelvic Floor Health", desc: "Exercises and recovery tips", color: "text-secondary-600", bg: "bg-secondary-50" },
          ].map((topic, i) => (
            <div key={i} className="bg-white p-6 rounded-2xl border border-slate-100 dark:border-dm-border hover:border-purple-200 hover:shadow-md transition-all cursor-pointer group flex gap-4">
              <div className={`w-12 h-12 rounded-xl ${topic.bg} ${topic.color} flex items-center justify-center shrink-0`}>
                <topic.icon size={24} />
              </div>
              <div>
                <h3 className="font-bold text-slate-900 dark:text-dm-foreground mb-1 group-hover:text-purple-600 transition-colors">{topic.title}</h3>
      ...
      {/* Warning Signs */}
      <div className="bg-gradient-to-br from-purple-50 to-secondary-50 rounded-[2rem] p-8 border border-purple-100">
        <div className="flex items-center gap-3 mb-6">
          <AlertCircle size={24} className="text-purple-500" />
          <h2 className="text-xl font-bold font-display text-slate-900 dark:text-dm-foreground">When to Seek Help</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            <div key={i} className="bg-white p-4 rounded-xl flex items-center gap-3">
              ...
              <span className="text-sm text-slate-600 dark:text-slate-300">{sign}</span>
            </div>
          ))}
        </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-6">
          <h2 className="text-xl font-bold font-display text-slate-900 dark:text-dm-foreground">Breastfeeding Resources</h2>
