...
export const Dashboard: React.FC<DashboardProps> = ({ phase, role }) => {
  ...
  // Cravings Tracker State
  const [cravings, setCravings] = useState<Array<{ id: number; food: string; intensity: 'mild' | 'moderate' | 'strong'; timestamp: Date; satisfied: boolean }>>([
      ...
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className="flex items-center justify-between p-4 rounded-xl border border-slate-100 dark:border-dm-border hover:bg-slate-50 dark:hover:bg-dm-muted transition-colors group cursor-pointer">
                    ...
                    <ArrowRight size={18} className="text-slate-300 dark:text-dark-600 group-hover:text-blue-500 transition-colors" />
                  </div>
                ))}
              </div>

              <button className="mt-6 w-full py-3 text-sm font-bold text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/10 rounded-xl transition-colors">
                + Add Task for Yourself
              </button>
            </div>

            {/* Goal Card */}
            <div className="bg-gradient-to-br from-blue-600 to-blue-700 dark:from-blue-700 dark:to-blue-900 rounded-[2rem] p-8 text-white relative overflow-hidden shadow-lg shadow-blue-900/20">
              <div className="relative z-10 max-w-xl">
                ...
                <div className="flex gap-4">
                  <button className="bg-white text-blue-700 dark:text-blue-900 px-6 py-3 rounded-xl font-bold text-sm flex items-center gap-2 hover:bg-blue-50 transition-colors">
          ...
          <div className="xl:col-span-4 space-y-6">
            {/* Tip of the Day */}
            <div className="bg-[#fffbeb] dark:bg-amber-900/20 rounded-[2rem] p-8 border border-amber-50 dark:border-amber-900/30">
              ...
              <div className="flex items-start gap-2">
                <p className="text-amber-900 dark:text-amber-200 italic text-sm leading-relaxed font-medium flex-1">
    ...
    return (
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 overflow-x-hidden">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-5 gap-6 mb-2">
          ...
          {/* Midwife AI Chat Card - Scoped Theme: Deep Plum */}
          {(() => {
            const PLUM_THEME = {
              background: '#673A51',
              accent: 'pink-300',
                ...
                <div className="relative z-10 flex flex-col h-full">
                  <div className="flex items-center gap-3 mb-6 border-b border-white/10 pb-4">
                    <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center border border-white/10">
                      <Lock size={16} className={`text-${PLUM_THEME.accent}`} />
                    </div>
                    <div className="flex-1">
                      <h3 className={`font-bold text-${PLUM_THEME.text} text-sm`}>Midwife AI</h3>
                      <div className="flex items-center gap-1.5">
                        <span className={`w-1.5 h-1.5 rounded-full bg-${PLUM_THEME.accent} animate-pulse`}></span>
                        <span className={`text-[10px] font-bold text-${PLUM_THEME.text}/60 uppercase tracking-wide`}>Private & Secure</span>
                      </div>
                    </div>
                    <SpeakButton text="Midwife AI. Your private and secure pregnancy assistant. Ask questions about your pregnancy journey." className={`text-${PLUM_THEME.text} border-white/30 bg-white/10 hover:bg-white/20`} size={12} />
                  </div>
                  <div ref={midwifeChatRef} className="flex-1 space-y-4 mb-4 overflow-y-auto custom-scrollbar pr-1">
                    {midwifeChatMessages.map((msg) => (
                      <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'gap-3'}`}>
                        <div className={`p-3 rounded-2xl text-xs leading-relaxed max-w-[85%] shadow-sm ${msg.sender === 'user'
                          ? `bg-${PLUM_THEME.button} text-${PLUM_THEME.text} rounded-tr-none`
                    ...
                    {isMidwifeChatLoading && (
                      <div className="flex gap-3">
                        <div className="bg-white/10 p-3 rounded-2xl rounded-tl-none text-xs text-secondary-100 border border-white/5 flex items-center gap-2 shadow-sm">
                          <Loader2 size={14} className={`animate-spin text-${PLUM_THEME.accent}`} />
                          Thinking...
                        </div>
                      </div>
                    )}
                  ...
                  <div className="relative mt-auto">
                    <input
                      ...
                      placeholder="Message..."
                      className={`w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-4 pr-12 text-sm text-${PLUM_THEME.text} focus:outline-none focus:ring-2 focus:ring-pink-400/50 transition-all placeholder:text-white/40`}
                    />
                    <button
                      onClick={handleMidwifeChatSend}
                      disabled={!midwifeChatInput.trim() || isMidwifeChatLoading}
                      className={`absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-${PLUM_THEME.button} text-${PLUM_THEME.text} rounded-lg flex items-center justify-center hover:bg-pink-400 transition-colors shadow-lg shadow-pink-900/20 disabled:opacity-50 disabled:cursor-not-allowed`}
                    >
                      <Send size={14} />
                    </button>
                  </div>
                  ...
                  <div className="flex gap-2">
                    {(['mild', 'moderate', 'strong'] as const).map((level) => (
                      <button
                        ...
                        className={`flex-1 py-3 rounded-xl text-xs font-bold capitalize transition-all ${newCravingIntensity === level
            ...
            {cravings.map((craving) => (
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${craving.intensity === 'mild' ? 'bg-green-100 text-green-700' :
                    craving.intensity === 'moderate' ? 'bg-amber-100 text-amber-700' :
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${craving.satisfied
