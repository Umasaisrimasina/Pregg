            ...
            <div className="relative z-10">
              ...
              <div className="flex justify-between max-w-md mx-auto mb-10">
                {[
                  ...
                  { icon: Smile, label: 'Good', color: 'bg-sky-100 text-sky-600', active: true },
                ].map((item, idx) => (
              ...
              <div className="mb-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {['Sleep', 'Work', 'Family', 'Baby Care', 'Diet'].map((tag, i) => (
                    <button key={i} className={`px-4 py-2 rounded-full text-sm border transition-colors ${tag === 'Baby Care' ? 'bg-sky-50 border-sky-200 text-sky-700 font-medium' : 'border-slate-200 text-slate-600 hover:border-sky-200'}`}>
                      {tag}
                    </button>
                  ))}
                </div>
              ...
              <div className="mt-6 flex justify-end">
                <button className="bg-sky-600 text-white px-8 py-3 rounded-xl font-bold text-sm shadow-lg shadow-sky-600/20 hover:bg-sky-700 transition-colors">
                  Save Entry
                </button>
              </div>
            </div>
